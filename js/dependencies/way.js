/* from: 
 * modified to work in a chrome extension
 */
(function (root, factory) {
	...
	WAY.prototype.getValue = function(element) {
		...
		var getters = {
			...
			"INPUT": function() {
				...
				if (_w.contains(["number", "text", "password"], type)) {
					return w.dom(element).val();
				}
	...
	WAY.prototype.setDefaults = function(force) {
		...
		for (var i in elements) {
			...
			if (!data || force) { self.dom(element).setDefault(); }
		}
	...
	*/
	...
	WAY.prototype.backup = function() {
		...
		try {
			var data = self.data || {};
			var temp = {};
			temp[tagPrefix] = JSON.stringify(data)
			chrome.storage.local.set(temp);
		} catch(e) {
			console.log("Your browser does not support chrome.storage.local.");
		}

	}

	WAY.prototype.restore = function(callback) {
		var self = this;
		if (!self.options.persistent) { return; }
		try {
			var data;
